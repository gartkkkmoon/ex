...
const EXACT_ANKR_CHAINS = [
  "eth", "polygon", "bsc", "arbitrum", "optimism", "base", "avalanche",
  "fantom", "gnosis", "linea", "scroll", "flare", "syscoin", "xlayer",
];

function exactAnkrEndpoint() {
  const token = String(exactRuntime.ankrToken || "").trim();
  return token ? `https://rpc.ankr.com/multichain/${token}` : "https://rpc.ankr.com/multichain";
}
...
async function exactAnkrAccountBalance(address) {
  ...
  let requestId = 1;
  do {
    const response = await fetch(exactAnkrEndpoint(), {
      method: "POST",
    ...
    if (!response.ok) throw new Error(`Ankr HTTP ${response.status}`);
    const payload = await response.json();
    if (payload.error) throw new Error(payload.error.message || "Ankr request failed");
    const result = payload.result || {};
    totalUsd += Number(result.totalBalanceUsd || 0) || 0;
    for (const item of result.assets || []) assets.push(item);
...
async function exactSyncLiveBalances() {
  ...
  try {
    ...
    exactRuntime.error = error.message || "Balance sync failed";
    exactRuntime.status = "Live sync failed — showing last known balances";
  } finally {
...
function renderRealHome() {
  ...
  const statusLine = exactIsLive()
    ? `<div class="real-live-status ${exactRuntime.error ? "error" : ""}">${
        exactRuntime.loading
          ? "Syncing balances…"
          : exactRuntime.error
            ? exactRuntime.error
            : `Live · ${exactShortHash(exactRuntime.address)}`
      }</div>`
    : "";
  return `
...
function renderRealWalletCard() {
  ...
  const statusLine = exactRuntime.error
    ? `<small class="real-wallet-error">${exactRuntime.error}</small>`
    : exactRuntime.status
      ? `<small>${exactRuntime.status}</small>`
      : "";
  return `
    <section class="real-wallet-card">
      ...
      <label class="real-field-label">Ankr API key <small>(optional)</small></label>
      <input class="real-text-input" data-runtime-field="ankrToken" placeholder="paste Ankr Advanced API key" value="${exactRuntime.ankrToken}" />
...
function exactStatusBar() {
  const ios = exactState.device !== "android";
  const icons = `
    <svg class="exact-sb-icons" width="118" height="24" viewBox="0 0 118 24" fill="#fff" aria-hidden="true">
      <rect x="0" y="14" width="5.5" height="9" rx="1.4"></rect>
      <rect x="8.5" y="10" width="5.5" height="13" rx="1.4"></rect>
      <rect x="17" y="6" width="5.5" height="17" rx="1.4"></rect>
      <rect x="25.5" y="1.5" width="5.5" height="21.5" rx="1.4"></rect>
      <path d="M44.5 7.6c5.4-4.8 14.6-4.8 20 0l-2.7 3c-3.9-3.4-10.7-3.4-14.6 0z"></path>
      <path d="M49.2 12.4c2.9-2.5 7.7-2.5 10.6 0l-2.7 3c-1.5-1.2-3.7-1.2-5.2 0z"></path>
      <circle cx="54.5" cy="20" r="2.5"></circle>
      <rect x="86" y="4" width="27" height="16" rx="4.5" fill="none" stroke="#fff" stroke-opacity="0.55" stroke-width="2"></rect>
      <rect x="88.5" y="6.5" width="19" height="11" rx="2"></rect>
      <rect x="114.5" y="9" width="2.6" height="6" rx="1.2" fill="#fff" fill-opacity="0.55"></rect>
    </svg>`;
  return `
    <div class="exact-statusbar ${ios ? "ios" : "android"}">
      <span class="exact-sb-time">9:41</span>
      ${ios ? `<span class="exact-sb-notch"></span>` : `<span class="exact-sb-punch"></span>`}
      <span class="exact-sb-right">${icons}</span>
    </div>
  `;
}

function exactHomeIndicator() {
  const ios = exactState.device !== "android";
  return ios
    ? `<div class="exact-home-indicator ios"><span></span></div>`
...
exactRoot.addEventListener("input", (event) => {
  ...
  if (event.target.matches("[data-runtime-field='ankrToken']")) {
    exactRuntime.ankrToken = event.target.value.trim();
    exactPersistRuntime();
  }

...
// Last-known activity list, cached separately from the runtime blob so it can
// be shown instantly on reopen instead of flashing empty while the fresh
// pending/history fetch resolves in the background.
const EXACT_TX_CACHE_KEY = "exx.exact.transactions.v1";
const EXACT_TX_CACHE_LIMIT = 60;
...
// Cache the current activity list so it shows immediately on reopen instead
// of flashing empty while the fresh pending/history fetch resolves. Scoped to
// the current address so a different restored/created wallet never inherits
// another wallet's cached transactions.
function exactPersistTransactions() {
  if (!exactIsLive() || !exactRuntime.address) return;
  try {
    window.localStorage?.setItem(EXACT_TX_CACHE_KEY, JSON.stringify({
      address: exactRuntime.address,
      transactions: exactState.transactions.slice(0, EXACT_TX_CACHE_LIMIT),
    }));
  } catch {
...
function exactRestoreTransactionsCache() {
  try {
    const raw = window.localStorage?.getItem(EXACT_TX_CACHE_KEY);
    if (!raw) return [];
    const saved = JSON.parse(raw);
    if (!saved || saved.address !== exactRuntime.address || !Array.isArray(saved.transactions)) return [];
...
function exactRestoreRuntime() {
  ...
  if (exactRuntime.mode === "live") {
    // Show the last-known activity immediately instead of an empty flash —
    // the fresh pending/history fetch reconciles it shortly after unlock.
    exactState.transactions = exactRestoreTransactionsCache();
  }
...
async function exactFetchPending() {
  ...
  if (exactApplyPending(list)) {
    exactPersistTransactions();
    exactRender();
  }
}
...
function exactHistoryTime(ms) {
  if (!ms) return "";
  return new Date(ms).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
}

// Pull the wallet's real confirmed on-chain history (all chains) and merge it
// into the activity list, replacing any session/pending entries that have since
// confirmed. Newest first.
async function exactFetchHistory() {
  if (!exactIsLive()) return;
  const params = new URLSearchParams();
  if (exactRuntime.address) params.set("evm", exactRuntime.address);
  if (exactRuntime.btcAddress) params.set("btc", exactRuntime.btcAddress);
  if (exactRuntime.ltcAddress) params.set("ltc", exactRuntime.ltcAddress);
  if (exactRuntime.dogeAddress) params.set("doge", exactRuntime.dogeAddress);
  if (exactRuntime.xrpAddress) params.set("xrp", exactRuntime.xrpAddress);
  ...
  let list = [];
  try {
    const res = await fetch(`/api/history?${params.toString()}`, { headers: { accept: "application/json" } });
    if (!res.ok) return;
    const data = await res.json();
    list = Array.isArray(data.transactions) ? data.transactions : [];
  } catch (error) {
    console.error("[exx] history fetch failed:", error);
    return;
  }
  if (exactMergeHistory(list)) {
    exactPersistTransactions();
    exactRender();
  }
}

function exactMergeHistory(list) {
  const confirmed = new Set(list.map((item) => item && item.hash).filter(Boolean));
  const before = JSON.stringify(exactState.transactions.map((t) => t.id));
  // Drop prior history entries and any session/pending tx that has now confirmed.
  exactState.transactions = exactState.transactions.filter(
    (tx) => !String(tx.id).startsWith("history-") && !confirmed.has(tx.hash),
  );
  const known = new Set(exactState.transactions.map((tx) => tx.hash));
  for (const item of list) {
    if (!item || !item.hash || known.has(item.hash)) continue;
    ...
    known.add(item.hash);
    exactState.transactions.push({
      id: `history-${item.hash}`,
      token: token.id,
      network: item.network || token.id,
      direction: incoming ? "incoming" : "outgoing",
      status: "confirmed",
      amount: `${incoming ? "+" : "-"}${exactFormatAmount(amountNum)} ${token.symbol}`,
      value: exactFormatUsd(amountNum * exactTokenPriceNumber(token)),
      fee: incoming ? "" : undefined,
      title: incoming ? "Received" : "Sent",
      day: exactHistoryDay(item.timestamp),
      time: exactHistoryTime(item.timestamp),
      address: item.counterparty || exactReceiveAddress(token),
      createdAt: item.timestamp || Date.now(),
      hash: item.hash,
    });
  }
  exactState.transactions.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
...
function exactResetWallet() {
  ...
  try {
    ...
    window.localStorage?.removeItem(EXACT_TX_CACHE_KEY);
  } catch {
    /* ignore */
  }
}
...
// Self-heal: wallets created before multi-chain support won't have SOL/XRP/TRX/
// DOGE/LTC addresses persisted. After unlock (when we can decrypt the seed),
// derive any that are missing so balances and receives work.
async function exactEnsureChainAddresses(pin) {
  if (!exactHasEncryptedSeed()) return;
  const fields = ["btcAddress", "solAddress", "xrpAddress", "trxAddress", "dogeAddress", "ltcAddress"];
  if (fields.every((f) => exactRuntime[f])) return; // all present
...
// A send that never made it on-chain at all (signing error, broadcast
// rejected — bad nonce, insufficient gas, RPC error, etc.). There's no txid,
// but the attempt is still recorded as Failed with the amount so it doesn't
// just vanish behind a toast.
function exactRecordFailedSend({ token, networkId, amount, to }) {
  const now = Date.now();
  const tx = {
    id: `failed-${now}-${Math.random().toString(16).slice(2)}`,
    token: token.id,
    network: networkId,
    direction: "outgoing",
    status: "failed",
    ...
    address: to,
    title: "Sent",
    day: exactHistoryDay(now),
    time: exactHistoryTime(now),
    createdAt: now,
    hash: "",
  };
  exactState.transactions.unshift(tx);
...
async function exactBroadcastAndRecord(senderFn, { token, network, pending }) {
  ...
  try {
    ...
    exactRuntime.loading = false;
    const tx = exactRecordFailedSend({ token, networkId: network.id, amount: pending.amount, to: pending.to });
    exactState.selectedTx = tx.id;
    exactState.screen = "txdetail";
    exactState.toast = "Transaction failed";
    exactPersistTransactions();
    exactRender();
  }
}
...
async function exactHandlePinComplete() {
  ...
  if (exactState.pinMode === "unlock") {
    ...
    exactState.screen = "home";
    exactRender();
    if (exactIsLive() && exactRuntime.unlocked) {
      await exactEnsureChainAddresses(entry);
  ...
  return `
    <main class="real-phone real-home exact-${exactState.device}">
      ...
      <nav class="real-bottom-dock">
        <button data-action="assets" aria-label="Assets">▣</button>
        <button data-action="activity" aria-label="Activity">⇄</button>
        <button data-action="open-access" aria-label="More">$</button>
      </nav>
      ${exactToast()}
    </main>
  `;
...
// Global activity / transaction history across every token.
function renderRealActivity() {
  const groups = exactActivityGroups(exactState.transactions);
  return `
    <main class="real-phone real-detail exact-${exactState.device}">
      <div class="real-scroll with-dock">
        <header class="real-detail-top">
          <button data-action="back" aria-label="Back">‹</button>
          <strong class="real-activity-title">Activity</strong>
          <button data-action="sync-balances" aria-label="Refresh">⟳</button>
        </header>
          ...
          ${groups.length ? groups.map((group) => `
            <div class="real-activity-group">
              ...
              ${group.items.map((tx) => {
                ...
                const token = exactTokens.find((entry) => entry.id === tx.token);
                return `
                <button class="real-tx-row ${tx.direction} ${tx.status} ${visual.tone}" data-action="txdetail" data-tx="${tx.id}">
                  ...
                  <strong>${visual.label} ${token ? token.symbol : ""}<small>${tx.time || exactShortHash(tx.hash)}</small></strong>
                  <em>${tx.amount}<small>${tx.value || ""}</small></em>
                </button>`;
              }).join("")}
            </div>
          `).join("") : `<p class="real-activity-empty">No transactions yet.<br/>Pull down to refresh once your wallet has activity.</p>`}
        </section>
      </div>
      <nav class="real-bottom-dock">
        <button data-action="home" aria-label="Wallet">▣</button>
        <button class="active" data-action="activity" aria-label="Activity">⇄</button>
        <button data-action="open-access" aria-label="More">$</button>
      </nav>
      ${exactToast()}
    </main>
  ...
  return `
    <main class="real-phone real-txdetail exact-${exactState.device}" style="--asset:${token.color}">
      ...
      <div class="real-scroll">
        ...
        ${failed ? `<section class="real-txd-card status"><span>Status</span><strong class="failed">✕ ${tx.hash ? "Failed — did not confirm" : "Failed to send"}</strong></section>` : ""}
        ${card(titleWord, `${amountClean}${tx.value ? `<small>${tx.value}</small>` : ""}`, "amount")}
        ${outgoing && !(failed && !tx.hash) ? card("Fee", tx.fee || `≈ 0.000029 ${exactNetworkFeeSymbol(token)}`) : ""}
...
function renderRealMobileScreens() {
  ...
  if (exactState.screen === "activity") return renderRealActivity();
...
exactRoot.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "activity") {
    exactState.screen = "activity";
    exactRender();
...
function exactLiveActive() {
  return exactIsLive() && exactRuntime.unlocked && !exactState.locked;
}
if (typeof setInterval === "function") {
  // Mempool pending receives — quick cadence so they pop without interaction.
  setInterval(() => { if (exactLiveActive()) exactFetchPending(); }, 20000);
